...
function ProductCard({ product, onAdd, isWishlisted, onToggleWishlist }: { product: Product; onAdd: (product: Product) => void; isWishlisted: boolean; onToggleWishlist: (product: Product) => void }) {
    ...
    <article className="group">
      <Link href={`/product/${product.handle}`} className="block">
        <div className="product-image relative overflow-hidden rounded-[1.35rem] bg-[#ece8df]">
          ...
          <button type="button" aria-label={`Save ${product.title}`} onClick={(event) => { event.preventDefault(); onToggleWishlist(product); }} className={`absolute right-3 top-3 rounded-full bg-[#fbfaf7]/90 p-2 transition hover:bg-white ${isWishlisted ? "text-[#a6604d]" : "text-[#37352f]"}`}><Heart size={16} strokeWidth={1.8} fill={isWishlisted ? "currentColor" : "none"} /></button>
        </div>
...
function CartDrawer() {
  ...
  return <div className="fixed inset-0 z-50"><button aria-label="Close cart" className="absolute inset-0 bg-[#25241f]/30 backdrop-blur-sm" onClick={closeCart} /><aside className="cart-drawer absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-[#fbfaf7] p-6 shadow-2xl sm:p-8">
    ...
    <div className="border-t border-[#ddd7cc] pt-5"><div className="flex justify-between text-sm text-[#777269]"><span>Subtotal</span><span className="font-semibold text-[#25241f]">{cart ? formatMoney(cart.subtotal) : "₹0"}</span></div><p className="mt-2 text-xs leading-5 text-[#969087]">Taxes and delivery are calculated securely by Shopify at checkout.</p><Link href="/cart" className="button-secondary mt-3 w-full justify-center">View full cart</Link><button className="button-primary mt-3 w-full justify-center" disabled={!cart?.itemCount || loading} onClick={proceedToCheckout}>Quick checkout with Shopify <ArrowRight size={16} /></button></div>
  </aside></div>;
}
...
export default function Home() {
  ...
  const wishlistAdd = trpc.commerce.wishlist.add.useMutation();
  const wishlistRemove = trpc.commerce.wishlist.remove.useMutation();
  const wishlistHandles = useMemo(() => new Set(wishlist.map(item => item.productHandle)), [wishlist]);
  ...
  const tags = useMemo(() => ["All tags", ...Array.from(new Set(products.flatMap(product => product.tags)))], [products]);
  const filtered = useMemo(() => filterAndSortProducts(products, { query, productType: activeType, tag, minPrice, maxPrice, inStockOnly, sort: sort as "featured" | "title-asc" | "title-desc" | "price-low" | "price-high" }), [products, query, activeType, sort, minPrice, maxPrice, tag, inStockOnly]);
  ...
  const toggleWishlist = async (product: Product) => { if (!isAuthenticated) { window.location.assign("/account"); return; } try { if (wishlistHandles.has(product.handle)) await wishlistRemove.mutateAsync({ productHandle: product.handle }); else await wishlistAdd.mutateAsync({ productHandle: product.handle }); await utils.commerce.wishlist.list.invalidate(); } catch { toast.error("We couldn't update your wishlist."); } };
  return <div className="min-h-screen bg-[#f7f4ee] text-[#25241f]">
    ...
    <main><section className="container hero-grid py-10 sm:py-14 lg:py-20"><div className="hero-copy"><p className="eyebrow">Everyday, elevated</p><h1 className="mt-5 max-w-2xl font-display text-6xl leading-[.93] tracking-[-.07em] sm:text-8xl">Find your <em>next</em> favourite.</h1><p className="mt-7 max-w-md text-base leading-7 text-[#6d6a61]">A thoughtful mix of useful, beautiful things—selected for the way you live now.</p><a href="#shop" className="button-primary mt-8 inline-flex">Explore the edit <ArrowRight size={16} /></a><div className="mt-12 flex items-center gap-4 text-xs uppercase tracking-[.16em] text-[#8d877b]"><span className="h-px w-12 bg-[#b7afa1]" /> New season / 2026</div></div><div className="hero-art"><div className="hero-orb" /><div className="hero-card hero-card-back" /><div className="hero-card hero-card-front"><span>LM</span><small>objects with intent</small></div><div className="hero-caption">A softer kind of shopping<br /><strong>made for everyday rituals</strong></div></div></section>
      <section id="shop" className="container pb-20"><div className="flex flex-col justify-between gap-5 border-b border-[#dcd6ca] pb-6 sm:flex-row sm:items-end"><div><p className="eyebrow">The current edit</p><h2 className="mt-3 font-display text-4xl tracking-[-.05em] sm:text-5xl">Good things, <em>well chosen.</em></h2></div><p className="max-w-xs text-sm leading-6 text-[#777269]">Live from the L-mart catalog. Inventory and availability update directly from Shopify.</p></div><div className="mt-7 flex flex-wrap items-center gap-2"><div className="flex gap-2 overflow-x-auto pb-2">{types.map(type => <button key={type} onClick={() => setActiveType(type)} className={`filter-pill ${activeType === type ? "filter-pill-active" : ""}`}>{type}</button>)}</div><label className="flex items-center gap-2 text-xs text-[#777269]">Tag <select aria-label="Filter by tag" value={tag} onChange={event => setTag(event.target.value)} className="rounded-full border border-[#d8d1c5] bg-transparent px-3 py-2 text-xs text-[#37352f] outline-none">{tags.map(option => <option key={option} value={option}>{option}</option>)}</select></label><label className="flex items-center gap-2 text-xs text-[#777269]"><input type="checkbox" checked={inStockOnly} onChange={event => setInStockOnly(event.target.checked)} className="accent-[#a6604d]" /> In stock</label><div className="ml-auto flex items-center gap-2"><label className="text-xs text-[#777269]">₹ <input aria-label="Minimum price" inputMode="numeric" value={minPrice} onChange={event => setMinPrice(event.target.value.replace(/[^0-9]/g, ""))} placeholder="Min" className="w-16 rounded-full border border-[#d8d1c5] bg-transparent px-3 py-2 text-xs outline-none" /></label><span className="text-xs text-[#aaa296]">–</span><label className="text-xs text-[#777269]">₹ <input aria-label="Maximum price" inputMode="numeric" value={maxPrice} onChange={event => setMaxPrice(event.target.value.replace(/[^0-9]/g, ""))} placeholder="Max" className="w-16 rounded-full border border-[#d8d1c5] bg-transparent px-3 py-2 text-xs outline-none" /></label><select aria-label="Sort products" value={sort} onChange={event => setSort(event.target.value)} className="rounded-full border border-[#d8d1c5] bg-transparent px-3 py-2 text-xs text-[#37352f] outline-none"><option value="featured">Featured</option><option value="title-asc">A–Z</option><option value="title-desc">Z–A</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select></div></div>{error ? <div className="mt-10 rounded-2xl bg-[#fff1ed] p-6 text-sm text-[#9a5145]">We couldn't load the live catalog. Please refresh and try again.</div> : isLoading ? <div className="product-grid mt-8">{[1, 2].map(item => <div key={item} className="skeleton-card" />)}</div> : filtered.length ? <div className="product-grid mt-8">{filtered.map(product => <ProductCard key={product.id} product={product} onAdd={addProduct} isWishlisted={wishlistHandles.has(product.handle)} onToggleWishlist={toggleWishlist} />)}</div> : <div className="mt-10 rounded-2xl border border-dashed border-[#d9d3c7] p-12 text-center text-sm text-[#777269]">No pieces match that search yet.</div>}</section>
      <RecentlyViewed />
